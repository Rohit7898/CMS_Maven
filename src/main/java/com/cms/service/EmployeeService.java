			...
			if(m.getN()==item) 
			{
				...
				else
				{
					String[] field= {"Order_id","item_id","user_id","quantity","date","time","token","Vendor_id","Tprice","Status","Msg"};
              		String[] values= {"103",Integer.toString(m.getItemId()),Integer.toString(id),"1",dtf.format(localDate),
              							sdf.format(cal.getTime()),"15",Integer.toString(m.getVendorId()),Float.toString(m.getItemPrice()),
              							"0",""};
